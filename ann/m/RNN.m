classdef RNN < handle
    ...
    properties (SetAccess = private)        % Access / GetAccess / SetAccess / SetObservable / Hidden
        ...
        transfer_deriv
        weights
        rweights                    % Recurrent weights
        recurrent_type = false
        biases
        activation
        output
        errors
    ...
    methods
        ...
        % Select network architecture (type)
        function SetNetworkType( this, type )
            switch ( type )
                case 'FFNN'     % Feed-forward NN
                    this.recurrent_type = false;
                case 'RNN'      % Recurrent NN
                    this.recurrent_type = true;
                otherwise
                    error(['Unknown network type specified --- ' type]);
            end
        ...
        function SetData( this, input_data, output_labels )
            ...
            if ( this.layer_nodes(1) ~= size(input_data,1) )
                warning('Input data must match input layer size (input data is expected to be of size: layer_nodes(1) x trials x time )');
                return;
            end
            if ( this.layer_nodes(end) ~= size(output_labels,1) )
                warning('Output labels must match output layer size (output labels are expected to be of size: layer_nodes(end) x trials x time )');
                return;
            end
            if ( size(input_data,2) ~= size(output_labels,2) )
                warning('Input and output trial counts must match (data and labels are expected to be of size: n x trials x time )');
                return;
            end
        ...
        function Initialise(this)
            if ( isempty(this.network_type) )
                this.SetNetworkType( 'RNN' );
            end
            ...
            switch ( method )
                case 'gaussian'
                    for layer = (2:(length(this.layer_nodes)))
                        ...
                        this.rweights{layer} = randn(this.layer_nodes(layer),this.layer_nodes(layer));      % Recurrent weights
                ...
                case 'xavier'               % Xavier Weight Initilisation (used for Sigmoid or TanH activation functions)
                    for layer = (2:(length(this.layer_nodes)))
                        ...
                        this.rweights{layer} = 2*bound*rand(this.layer_nodes(layer),this.layer_nodes(layer))-bound;
        ...
        function ForwardPropagate(this)
            ...
            for layer = (2:length(this.layer_nodes))
                t=1;
                this.activation{layer}(:,:,t) = this.weights{layer}*this.output{layer-1}(:,:,t) + this.biases{layer};
                this.output{layer}(:,:,t) = this.transfer( this.activation{layer}(:,:,t) );
                for t = (2:size(this.input_data,3))
                    this.activation{layer}(:,:,t) = this.weights{layer}*this.output{layer-1}(:,:,t) + this.rweights{layer}*this.output{layer}(:,:,t-1) + this.biases{layer};
                    this.output{layer}(:,:,t) = this.transfer( this.activation{layer}(:,:,t) );
                end
            end
            this.cost = mean( mean( sum((this.output_labels - this.output{end}).^2,1), 2), 3);
        end
