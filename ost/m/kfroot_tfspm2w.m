...
% Initial conditions
xp(:,1)=reshape(sp(:,(1:2),1),vlen,1);	% Initial 'a priori' state vector
xp12(:,1)=[sp(:,3,1); sp(:,4,1)];       % Initial 'a priori' cross-spectra (not indexed)
...
%     Pp(:,ind) = P(:,ind-1)+Q;       % Update `a priori' state covar
    ...
    newz12=[sp(:,3,ind); sp(:,4,ind)];                                  % Cross-periodogram (new measurement)
