...
bias = log(cl(1).seg_tot) - psi(0,cl(1).seg_tot);
xp(:,1)=[ [sp(:,1,1); sp(:,2,1)]+bias; sp(:,3,1)];                  % Initial 'a priori' state vector
R=[ones(2*fpts,1)*varstab*psi(1,cl(1).seg_tot); ones(fpts,1)/(2*cl(1).seg_tot-2)];	% Measurement error-covariance matrix
...
for ind=(2:M)
    ...
    bias = log(cl(ind).seg_tot) - psi(0,cl(ind).seg_tot);
    cohbias = 0;%- 1./L((2*end/3+1):end,ind);
    newz = [ [ sp(:,1,ind); sp(:,2,ind) ] + bias; sp(:,3,ind) + cohbias ];        % Bias correction in estimator
    % Construct measurement noise covariance (from updated spectrum)
    R = [ ones(2*fpts,1)*varstab*psi(1,cl(ind).seg_tot); ones(fpts,1)/(2*cl(ind).seg_tot-2) ];
    % Evidence
