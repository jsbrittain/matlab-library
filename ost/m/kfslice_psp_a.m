function kfslice_psp_a(sp,params,freq,label)
%function kfslice_psp_a(sp,params,freq,[label])
%
...
switch (params.method)
    case 0          % null-smoothing - use periodogram noise (provided)
        ...
        %cl95=1.96*squeeze(sqrt(double(params.Psp(fpos,ch,:))));
    ...
    case 6          % kflog - adaptive with full covar matrices (NB: SINGLE CHANNEL)
        cl95=(10*log10(exp(1)))*(1.96*squeeze(sqrt(double(params.Psp(fpos,fpos,:)))));
        cl95_lower=faa-cl95; cl95_upper=faa+cl95;
        if (isfield(params,'jk'))
            jkcl95=(10*log10(exp(1)))*(1.96*squeeze(sqrt(params.jk(fpos,fpos,:))));
            jkcl95_lower=faa-jkcl95; jkcl95_upper=faa+jkcl95;
        end;
end;
