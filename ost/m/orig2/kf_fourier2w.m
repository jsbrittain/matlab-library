...
for ind=1:M
    ...
    segdat1=dat1((newtrig(1)+offset):(newtrig(end)+offset+duration));
    segdat2=dat2((newtrig(1)+offset):(newtrig(end)+offset+duration));
    newtrig=newtrig-newtrig(1)-offset;
    
    % Perform analysis over group
    disp(['Segment group ' int2str(ind) ' of ' int2str(M)]);
    [f(:,:,ind),t(:,:,ind),cl(ind),sp(:,:,ind)]=sp2a2_ma(segdat1,segdat2,newtrig,offset_ms,duration_ms,rate,seg_pwr,opt_str);
end;
