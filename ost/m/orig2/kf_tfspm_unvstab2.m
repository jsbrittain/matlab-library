...
for ind=2:M
    ...
    if p_range(1)<1
            p_range=p_range-p_range(1)+1;
    else
            if p_range(2)>M
                p_range=p_range-p_range(2)+M;
            else
                power=power-sp(:,:,p_range(1))/p_width+sp(:,:,p_range(2))/p_width;
                p_cov(:,ind,1)=std(squeeze(sp(:,1,p_range(1):p_range(2)))')'./power(:,1);
                p_cov(:,ind,2)=std(squeeze(sp(:,2,p_range(1):p_range(2)))')'./power(:,2);
            end;
    end;
