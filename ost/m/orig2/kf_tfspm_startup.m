...
% Issues:
%       bounding        Coherence outside [0,1] for large Q%age
...
R=(x(:,1).^2)./cl(1).seg_tot;           % Measurement error-covariance matrix
Pp(:,1)=Q.*R;
...
for ind=1:M
    ...
    R(2*vlen/3+1:vlen)=spec(1:vlen/3).*spec(vlen/3+1:2*vlen/3);
    % Kalman filter
