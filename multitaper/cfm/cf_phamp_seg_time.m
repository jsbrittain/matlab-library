function params = cf_phamp_seg_time( preprocdata, trimlevel, bincount, permutation_count, segs )
%
%
...
magtime = 1 + floor( (0:(length(magxh)-1))*segs/length(magxh) ).';
...
for ind = (1:(permutation_count+1))
    ...
    for k = (1:segs)
        ...
        for n = (0:(bincount-1))
            rmag(ind,k,n+1) = trimmean( magxh((anglegrid==n) & (magtime==k)), trimlevel );
            rmagsd(ind,k,n+1) = std( magxh((anglegrid==n) & (magtime==k)) );
            rif(ind,k,n+1) = trimmean( ifxh((anglegrid==n) & (magtime==k)), trimlevel );
            rcount(ind,k,n+1) = sum((anglegrid==n) & (magtime==k));
        end;
