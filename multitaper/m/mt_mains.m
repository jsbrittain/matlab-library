...
for lineind=lineind
    ...
    % Smooth elements in upper hermitian section of cross spectral estimate.
    if (length(find(freqs<0))>0)
        ...
        sp(lineindneg+1)=conj(sp(lineind));
        sp(lineindneg+2)=conj(sp(lineind-1));
        sp(lineindneg+0)=conj(sp(lineind+1));
    end;
