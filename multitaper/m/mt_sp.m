...
%                       p<n>        zero pad to 2^n, or -n msecs (default: next highest)
...
%                       c           Return complex valued spectra (meant
%                                     for single-trial decompositions)
...
%
% To account for spectral spreading at higher frequencies when warping, all
% spectra are scaled by 1/df (regardless of warping)
%
%function [sp11,[sp22,sp12],params]=mt_sp(dat1,[dat2],rate,opt_str,[v,dk]);
...
if ( pad )
    if ( seg_pwr > 0 )
        ...
        padding=zeros(-seg_pwr*rate/1000-length(dat1),1);
    end;
    dat1=[dat1; padding];
...
if ( warp_signal )
    ...
    freqs0 = freqs0(1:floor(length(freqs0)/10):length(freqs0));
    wfreqs0 = freqs0;
    for n = (1:length(freqs0))
        freq0 = freqs0(n);
        wf1 = inline(['abs( atan2((1-' num2str(warping_lambda) '^2)*sin(x),((1+' num2str(warping_lambda) '^2)*cos(x)-2*' num2str(warping_lambda) ')) - ' num2str(freq0*2*pi/rate) ');']);
        wfreqs0(n) = fminbnd( wf1, 0, pi  )*rate/2/pi;
    end;
    freqs = sign(freqs).*interp1( freqs0, wfreqs0, abs(freqs), 'spline' );
    
end;
df = [ freqs(2)-freqs(1) diff(freqs) ].';
...
%     end;
    ...
    if (~complexspectra)
        p11(:,taper)=(abs(F1).^2)./df/spec_norm;
        if (bivariate)
            p22(:,taper)=(abs(F2).^2)./df/spec_norm;
            p12(:,taper)=(F2.*conj(F1))./df/spec_norm;       % f21 not f12 for cumulant direction
        end;
    else
        p11(:,taper)=F1./df/spec_norm;
        if (bivariate)
            p22(:,taper)=F2./df/spec_norm;
            p12(:,taper)=F2.*conj(F1)./df/spec_norm;
        end;
    end;
end;
