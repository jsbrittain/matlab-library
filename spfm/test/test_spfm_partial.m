...
figure; fig2a4l;
subplot(2,3,1); psp_fa1(f(:,:,3),cl(3),100); ylim([-2.42 -2.26]); xlabel('FREQ (Hz)'); title('SPECTRUM NEURON A');
subplot(2,3,2); psp_ch1(f(:,:,3),cl(3),100,0.1); ylim([0 0.05]); xlabel('FREQ (Hz)'); title('COHERENCE');
subplot(2,3,3); psp_q1(t(:,:,3),cl(3),80,40); axis('tight'); ylims=ylim; xlabel('OFFSET (msecs)'); title('CUMULANT DENSITY');
subplot(2,3,4); psp_fa1(fp,clp,100); ylim([-2.42 -2.26]); xlabel('FREQ (Hz)'); title('PARTIAL SPECTRUM NODE A');
subplot(2,3,5); psp_ch1(fp,clp,100,0.1); ylim([0 0.05]); xlabel('FREQ (Hz)'); title('PARTIAL COHERENCE');
subplot(2,3,6); psp_q1(tp,clp,80,40); ylim(ylims); xlabel('OFFSET (msecs)'); title('PARTIAL CUMULANT DENSITY');
