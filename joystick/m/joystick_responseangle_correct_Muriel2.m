function [rangle,joy] = joystick_responseangle_correct_Muriel2( rangle, joy, filterpts100, startoffset )
...
while ( n <= length(joy) )
    ...
    % Crossing
    if ( isempty(joy(n).update) )
        velpt0 = joy(n).rtvelpt;
        if (isempty(joy(n).rtrange))
            joy(n).rtrange = [ 1 length(fvel) ];
        end;
        joy(n).rtvelpt = find( ((fvel.') > crossingTh) & ((1:length(fvel)) > joy(n).rtrange(1)) & ((1:length(fvel)) < joy(n).rtrange(2)), 1, 'first' );
        if (isempty(joy(n).rtvelpt))
            joy(n).rtvelpt = velpt0;
        end;
    end;
...
%     fvel = filtfilt(b,a,vel);
    ...
    choice = input( ' continue (ENTER), back (b), peak velocity (p), search range (s), reject (r), filter length (f), vel-crossing th (t), save & exit (x) ? ', 's' );
    ...
    elseif strcmpi(choice,'t')   % Change threshold
        disp(' WARNING: This changes the threshold on ALL TRIALS.');
        crossingTh = str2num( input(['  Enter new threshold (currently ' num2str(crossingTh) ' pt): '],'s') );
