function [q,s2,evidence,ct,rhoavg,S,q0] = var_kalman_efficient(y,p,s2static,alpha,smoother)
%
...
% Kalman Smoother
if (smoother>0)
    warning(' Smoother not yet implemented!');
end;
switch (smoother)
    case 1,         % Forward-backward smoother
        percent = 0;
        for t=((N-1):(p+2))
            % Display progress
            if (round(t/N*100)<percent)
                percent=round(t/N*100);
                disp(['Kalman fiter ' num2str(100-percent) '%'])
            end;
            % Backward gain
            A=P(:,ind)./Pp(:,ind+1);
            xN(:,ind)=x(:,ind)+A.*(q(:,ind+1)-xp(:,ind+1));
