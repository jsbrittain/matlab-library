function [q,s2,evidence,ct,rhoavg] = var_kalman(y,p,s2static,alpha)
%
...
S(:,:,p+1) = trace(s2static*(Ft*(Ft.')))/m * eye(m2p);
%S(:,:,p+1) = eye(m2p);
...
for t=((p+2):N)
        ...
        case 0,
            ...
            if ( alpha < 0 )
               ...
               %ct(:,:,t) = (ert*ert'-s2q0)/(Ft*(Ft.')); ct(ct<0)=0;% Learning rate
                ct(:,:,t) = pinv(Ft)*(ert*ert'-s2q0)*pinv(Ft');
                ct(:,:,t) = diag(diag( (ct(:,:,t)>0).*ct(:,:,t) ));
                ct(:,:,t) = alpha*ct(:,:,t-1)+(1-alpha)*ct(:,:,t);  % Smoothed learning rate
                Wt = ct(:,:,t);                                     % Isotropic (c.I) matrix
            end;
        case 1,
            % Arnold
    end;
    ...
    K(:,:,t) = ((S(:,:,t-1) + Wt)*(Ft.'))*pinv(s2y(:,:,t));         % Kalman gain
