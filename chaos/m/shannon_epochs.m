function [shannon,shannonnout] = shannon_epochs(data,dt,bins,process,offset,rectify)
%
%
%
...
for tt=trange
    ...
    for ind=(1:size(data,3))
        ...
        nout=nout+n{ind2,ind};
    end;
    nout=nout/sum(nout);        % -> PDF
    pdf(:,ind2)=nout;
    ...
    % Shannon entropy
    nout(nout==0) = 1;
    shannonnout(:,ind2)=nout;
    shannon(ind2) = -sum(nout.*log2(nout));
...
for ind2=(1:size(n,1))
    for ind1=(1:size(n,2))
        ...
        pv(ind2,ind1)=N*shannon(ind2)-(N-1)*sh;     % Pseudovalue
    end;
end;
for ind2=(1:size(n,1))
...
subplot(3,1,2); if (rectify)
		    ...
		    axis('tight');
                ...
                title('MEAN RESPONSE');
subplot(3,1,3); plot(dt/2+[1:dt:size(data,2)-dt]-offset,shannon,'k'); hold on;
