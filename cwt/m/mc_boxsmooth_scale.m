...
nbins=1000;
[W1,wlparam1]=wlTransform(randn(N,1),dt,dj,srange,opt,mother,wlopt);
sizeW1=size(W1);
clear('W1');
outsidecoi=zeros(sizeW1);
for ind=1:length(wlparam1.scale)
    outsidecoi(ind,:)=(wlparam1.scale(ind)<=wlparam1.coi);
end;
maxscale=1;
for ind=1:length(wlparam1.scale)
...
for trial=1:trials
    ...
    % Scale spectra - CORRECTION: PERFORMED IN WLTRANSFORM
%     for ind=1:length(wlparam1.scale)
